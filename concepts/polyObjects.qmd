...
have its own timeline state too, including ruler display settings and
snap configuration, so you may want to set those according to the local
editing task. A PolyObject can therefore be edited with a different
primary or secondary ruler display than the root score while still
remaining synchronized to the same musical material. Add SoundLayers
just as you would in the main score too.
...
The Repeat Point specifies the time span between loop starts, and it may be entered
using any supported duration time base. If the Use Repeat Point is not
checked, the next note group will begin immediately after the previous
note group's last note ends.
...
set the repeat point to `2.5` beats, this would be the result:
...
If you prefer to think in bars and beats rather than raw beat counts,
you can enter the repeat point using the same time-aware duration editor
used elsewhere in the Score. For the broader rules around ruler formats,
stored time bases, and duration entry, see
[Time System](./timeSystem.qmd).
